...
func runClientCommandFunc(cmd *cobra.Command, args []string, doTransactions bool, command string) {
	...
	rps := strconv.Itoa(targetArg)
	file0, err := os.Create("/var/www/html/ycsb/rps.txt")
	if err != nil {
		log.Fatal(err)
	}
	writer0 := bufio.NewWriter(file0)
	_, err = writer0.WriteString(rps)
	writer0.Flush()
	file0.Close()
	...
	measurement.Output()
	file, err := os.Create("/var/www/html/ycsb/p99.txt")
	if err != nil {
		log.Fatal(err)
	}
